...
// Existing QR Attendance Endpoint
router.post("/attendance/qr/:studentId", async (req, res) => {
  try {
    const { QR_ATTENDANCE_TOKEN } = req.body;
    console.log('Received QR_ATTENDANCE_TOKEN:', QR_ATTENDANCE_TOKEN);
    console.log('Expected QR_ATTENDANCE_TOKEN:', process.env.QR_ATTENDANCE_TOKEN);
    if (!QR_ATTENDANCE_TOKEN || QR_ATTENDANCE_TOKEN !== process.env.QR_ATTENDANCE_TOKEN) {
...
router.post("/attendance/mark", authenticateToken, async (req, res) => {
  try {
    ...
    const attendanceRecord = {
      ...
      timeIn: now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      timeOut: null,
      notes: "Marked via static QR code scan",
    };
    student.attendance.push(attendanceRecord);
    admin.attendance.push({
      ...attendanceRecord,
      notes: `Marked as present for student: ${studentName}`,
    });
    await student.save();
    await admin.save();
    res.status(201).json({
      message: "Attendance marked successfully for student and admin",
      attendance: attendanceRecord,
    });
  } catch (error) {
...
// Record Student Attendance (Admin Manual)
